...
const runner = new TestRunner('Provider/Gemini(E2E)');
const env = loadProviderEnv('gemini');

if (!env.ok || !env.config) {
  runner.skip(`Gemini E2E 跳过：${env.reason}`);
} else {
  const apiKey = env.config.apiKey;
  const model = env.config.model || 'gemini-3.0-flash';
  const baseUrl = env.config.baseUrl;
  const proxyUrl = env.config.proxyUrl;

  runner
    .test('正常输出（流式）', async () => {
      const ctx = await createProviderTestAgent({ provider: 'gemini', apiKey, model, baseUrl, proxyUrl, tools: [] });
      try {
        const result = await runChatWithEvents(ctx.agent, '你好');
        assertTextStream(result.progress, 'gemini:e2e-normal');
    ...
    .test('工具发现', async () => {
      const ctx = await createProviderTestAgent({ provider: 'gemini', apiKey, model, baseUrl, proxyUrl, tools: ['always_ok'] });
      try {
        const result = await runChatWithEvents(ctx.agent, '请列出当前可用的工具名称。');
        assertHasText(result.progress, 'gemini:e2e-tools');
        expect.toEqual(ctx.monitorErrors.length, 0);
      } finally {
        await ctx.cleanup();
      }
    ...
    .test('工具调用成功', async () => {
      ...
      try {
        ...
        assertToolSuccessFlow(result.progress, 'gemini:e2e-tool-success');
        expect.toEqual(ctx.monitorErrors.length, 0);
      } finally {
        await ctx.cleanup();
      }
    ...
    .test('工具调用失败', async () => {
      const ctx = await createProviderTestAgent({ provider: 'gemini', apiKey, model, baseUrl, proxyUrl });
      try {
        const result = await runChatWithEvents(ctx.agent, '请调用 always_fail 工具，reason=forced。');
        assertToolFailureFlow(result.progress, 'gemini:e2e-tool-fail');
        const hasForcedError = ctx.monitorErrors.some((evt) => String(evt.message).includes('forced'));
        expect.toBeTruthy(hasForcedError, '[gemini:e2e-tool-fail] Missing monitor error');
      } finally {
        await ctx.cleanup();
      }
    })
    .test('多轮交互', async () => {
      const token = 'CTX-GEMINI-42';
      const ctx = await createProviderTestAgent({ provider: 'gemini', apiKey, model, baseUrl, proxyUrl, tools: [] });
      try {
    ...
    .test('权限请求（approval -> allow）', async () => {
      ...
      try {
        ...
        assertPermissionRequired(result.control, 'gemini:e2e-permission');
        assertToolSuccessFlow(result.progress, 'gemini:e2e-permission');
        expect.toEqual(ctx.monitorErrors.length, 0);
      } finally {
        await ctx.cleanup();
      }
    ...
    .test('权限请求（approval -> deny）', async () => {
      ...
      try {
        ...
        assertPermissionRequired(result.control, 'gemini:e2e-permission-deny');
        const types = result.progress.map((event) => event.type);
        expect.toBeTruthy(types.includes('tool:start'), '[gemini:e2e-permission-deny] Missing tool:start');
        expect.toBeTruthy(types.includes('tool:end'), '[gemini:e2e-permission-deny] Missing tool:end');
        expect.toBeFalsy(types.includes('tool:error'), '[gemini:e2e-permission-deny] Unexpected tool:error');
        const endEvent = result.progress.find((event) => event.type === 'tool:end') as any;
        expect.toBeTruthy(endEvent?.call?.isError, '[gemini:e2e-permission-deny] tool:end should be error');
        expect.toEqual(ctx.monitorErrors.length, 0);
      } finally {
        await ctx.cleanup();
      }
